...
(function (root, factory) {
  ...
  var removeCollectionProp = function removeCollectionProp(
    collection,
    unwantedProp,
  ) {
  ...
  function isObject(obj) {
    ...
    return obj === Object(obj) && type != "Array" && type != "Function" &&
      type != "RegExp" && type != "HTMLUnknownElement";
  }
  ...
  Tagify.prototype = {
    ...
    templates: {
      ...
      wrapper: function wrapper(input, settings) {
        return '<tags class="tagify '.concat(
          settings.mode ? "tagify--" + settings.mode : "",
          " ",
        ...
        ).concat(settings.placeholder || "&#8203;", '" aria-placeholder="')
          .concat(
      ...
      dropdown: function dropdown(settings) {
        var _s = settings.dropdown,
          className = "".concat(
            _s.position == "manual"
              ? ""
              : "tagify__dropdown tagify__dropdown--".concat(_s.position),
            " ",
          ).concat(_s.classname).trim();
    ...
    applySettings: function applySettings(input, settings) {
      ...
      this.settings.placeholder = input.getAttribute("placeholder") ||
    ...
    getAttributes: function getAttributes(data) {
      ...
      for (i = keys.length; i--;) {
        propName = keys[i];
        if (
          propName != "class" && data.hasOwnProperty(propName) && data[propName]
        ) {
          s += " " + propName +
            (data[propName] ? '="'.concat(data[propName], '"') : "");
        }
      }
    ...
    events: {
      ...
      binding: function binding() {
        var bindUnbind = arguments.length > 0 && arguments[0] !== undefined
          ? arguments[0]
          : true;
      ...
      callbacks: {
          ...
          if (this.settings.mode == "mix") {
            switch (e.key) {
              ...
              case "Backspace": {
                ...
                if (
                  selection.anchorNode.nodeType == 3 && // node at caret location is a Text node
                ...
                setTimeout(function () {
                  ...
                  if (
                    decode(_this4.DOM.input.innerHTML).length >=
                      lastInputValue.length
                  ) {
                    _this4.removeTag(
                      selection.anchorNode.previousElementSibling,
                    ...
                    if (
                      _this4.DOM.input.children.length == 2 &&
                      _this4.DOM.input.children[1].tagName == "BR"
                    ) {
        ...
        onInput: function onInput(e) {
          var value = this.settings.mode == "mix"
        ...
        onMixTagsInput: function onMixTagsInput(e) {
          ...
          if (window.getSelection) {
            ...
            if (
              selection.rangeCount > 0 && selection.anchorNode.nodeType == 3
            ) {
              ...
              if (tag) {
                ...
                this.state.tag.baseOffset = selection.baseOffset -
                  this.state.tag.value.length;
                showSuggestions =
                  this.state.tag.value.length >= _s.dropdown.enabled; // When writeing something that might look like a tag (an email address) but isn't one - it is unwanted
          ...
          setTimeout(function () {
            ...
            _this5.trigger(
              "input",
              extend({}, _this5.state.tag, {
                textContent: _this5.DOM.input.textContent,
              }),
            );

            if (_this5.state.tag) {
              _this5.dropdown[showSuggestions ? "show" : "hide"].call(
                _this5,
                _this5.state.tag.value,
              );
            }
          }, 10);
        },
        ...
        onDoubleClickScope: function onDoubleClickScope(e) {
          ...
          if (
            _s.mode != "select" && !_s.readonly && !isEditingTag &&
            !isReadyOnlyTag && this.settings.editTags
          ) this.editTag(tagElm);
    ...
    editTag: function editTag(tagElm, opts) {
      ...
      if (
        tagData instanceof Object && "editable" in tagData && !tagData.editable
      ) return; // cache the original data, on the DOM node, before any modification ocurs, for possible revert
    ...
    onEditTagDone: function onEditTagDone(tagElm, tagData) {
      ...
      if (tagData.__isValid === true) {
        this.getTagElms("tagify--notAllowed").forEach(function (notAllowedTag) {
    ...
    input: {
      ...
      set: function set() {
        var s = arguments.length > 0 && arguments[0] !== undefined
          ? arguments[0]
          : "";
        var updateDOM = arguments.length > 1 && arguments[1] !== undefined
          ? arguments[1]
          : true;
      ...
      validate: function validate() {
        ...
        } else {this.DOM.input.classList.toggle(
      ...
      autocomplete: {
        suggest: function suggest(data) {
          ...
          var suggestedText = data.value || "",
            suggestionStart = suggestedText.substr(0, this.input.value.length)
              .toLowerCase(),
          ...
          if (
            !suggestedText || !this.input.value ||
            suggestionStart != this.input.value.toLowerCase()
          ) {
        ...
        set: function set(s) {
          var dataSuggest = this.DOM.input.getAttribute("data-suggest"),
            suggestion = s ||
              (dataSuggest ? this.input.value + dataSuggest : null);
    ...
    validateTag: function validateTag(tagData) {
      ...
      if (!value) result = this.TEXTS.empty; // check if pattern should be used and if so, use it to test the value
        ...
        _s.pattern && _s.pattern instanceof RegExp && !_s.pattern.test(value)
      ) result = this.TEXTS.pattern; // if duplicates are not allowed and there is a duplicate
      else if (!_s.duplicates && this.isTagDuplicate(value)) {
        result = this.TEXTS.duplicate;
      } else if (
        this.isTagBlacklisted(value) ||
        _s.enforceWhitelist && !this.isTagWhitelisted(value)
    ...
    normalizeTags: function normalizeTags(tagsItems) {
      var _this$settings = this.settings,
        ...
        isCollection = isArray && tagsItems[0] instanceof Object &&
          "value" in tagsItems[0],
        temp = [],
      ...
      if (isCollection) {
        ...
        tagsItems = (_ref2 = []).concat.apply(
          _ref2,
          _toConsumableArray(tagsItems.map(function (item) {
    ...
    parseMixTags: function parseMixTags(s) {
      ...
      s = s.split(mixTagsInterpolator[0]).map(function (s1, i) {
        ...
        if (
          s2.length > 1 &&
          (!enforceWhitelist || _this10.isTagWhitelisted(tagData.value)) &&
          !(!duplicates && _this10.isTagDuplicate(tagData.value))
        ) {
    ...
    addTags: function addTags(tagsItems, clearInput) {
      ...
      var skipInvalid = arguments.length > 2 && arguments[2] !== undefined
        ? arguments[2]
      ...
      if (_s.mode == "mix") {
        ...
        tagsItems[0].prefix = tagsItems[0].prefix || this.state.tag
      ...
      tagsItems.forEach(function (tagData) {
        ...
        tagData.__isValid = _this11.hasMaxTags() ||
          _this11.validateTag(tagData);

        if (tagData.__isValid !== true) {
          if (skipInvalid) return;
          extend(
            tagElmParams,
            _this11.getInvaildTagParams(tagData, tagData.__isValid),
          ); // mark, for a brief moment, the tag THIS CURRENT tag is a duplcate of

          if (tagData.__isValid == _this11.TEXTS.duplicate) {
            _this11.markTagByValue(tagData.value);
          }
        } /////////////////////////////////////////////////////
    ...
    reCheckInvalidTags: function reCheckInvalidTags() {
      ...
      var tagElms = this.DOM.scope.querySelectorAll(
        ".tagify__tag.tagify--notAllowed",
      );
      [].forEach.call(tagElms, function (node) {
        var tagData = node.__tagifyTagData,
          wasNodeDuplicate =
            node.getAttribute("title") == _this12.TEXTS.duplicate,
    ...
    removeTag: function removeTag(tagElm, silent, tranDuration) {
      ...
      function animation() {
        tagElm.style.width = parseFloat(window.getComputedStyle(tagElm).width) +
          "px";
    ...
    removeAllTags: function removeAllTags() {
      ...
      else {Array.prototype.slice.call(this.getTagElms()).forEach(
          function (elm) {
        ...
        rootNode.childNodes.forEach(function (node) {
          if (node.nodeType == 1) {
            if (node.classList.contains("tagify__tag")) {
              result += _interpolator[0] + JSON.stringify(currentTags[i++]) +
                _interpolator[1];
              return;
    ...
    dropdown: {
      ...
      show: function show(value) {
        ...
        if (
          noWhitelist && !allowNewTags || _s.dropdown.enable === false ||
        ...
        if (this.suggestedListItems.length) {
          if (
            value && allowNewTags && !this.state.editing.scope &&
        ...
        firstListItemValue = isObject(firstListItem)
          ? firstListItem.value
          : firstListItem;
        ...
        HTMLContent = this.dropdown.createListHTML.call(
          this,
          this.suggestedListItems,
        );
        ...
        if (!document.body.contains(this.DOM.dropdown)) {
          if (!isManual) {
            ...
            setTimeout(function () {
              return _this13.DOM.dropdown.classList.remove(
      ...
      refilter: function refilter() {
        ...
        var listHTML = this.dropdown.createListHTML.call(
          this,
          this.suggestedListItems,
        );
      ...
      position: function position(ddHeight) {
        ...
        isBelowViewport = document.documentElement.clientHeight - bottom <
          (ddHeight || ddElm.clientHeight); // flip vertically if there is no space for the dropdown below the input

        ddElm.style.cssText = "left:" + (left + window.pageXOffset) +
          "px; width:" + width + ";" + (isBelowViewport
            ? "bottom:" +
      ...
      events: {
        ...
        binding: function binding() {
          ...
          this.DOM[this.listeners.main.click[0]][action](
            "click",
        ...
        callbacks: {
          onKeyDown: function onKeyDown(e) {
            ...
            var activeListElm = this.DOM.dropdown.querySelector(
                "[class$='--active']",
              ),
          ...
          onScroll: function onScroll(e) {
            var elm = e.target,
              pos = elm.scrollTop /
                (elm.scrollHeight - elm.parentNode.clientHeight) * 100;
      ...
      highlightOption: function highlightOption(elm, adjustScroll) {
        ...
        if (adjustScroll) {
          elm.parentNode.scrollTop = elm.clientHeight + elm.offsetTop -
            elm.parentNode.clientHeight; // Try to autocomplete the typed value with the currently highlighted dropdown item
        }
      ...
      selectOption: function selectOption(elm) {
        ...
        var hideDropdown = this.settings.dropdown.closeOnSelect,
          value = this.suggestedListItems[this.getNodeIndex(elm)] ||
            this.input.value;
      ...
      filterListItems: function filterListItems(value) {
        ...
        if (!value) {
          return (
            _s.duplicates ? whitelist : whitelist.filter(function (item) {
              return !_this15.isTagDuplicate(
